...
public class PersonTypeDAO {
    public static List getAll() {
        ...
        so.beginTransaction();
        List result = so.createQuery("from PersonType").list();
    ...
    public static List getAllById(long Id) {
        var so = BaseDB.openConnection();
        so.beginTransaction();
        List result = so.createQuery("from PersonType where Id = " + String.valueOf(Id)).list();
        so.getTransaction().commit();
        so.close();
        return result;
    }
    ...
    public static List getAllByFilter(Filter filter) {
        StringBuilder sql = new StringBuilder("from PersonType ");
